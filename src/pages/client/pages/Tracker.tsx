        ...
        const fetchClientData = async () => {
            try {
                ...
                if(res.data.success) {
                    ...
                    if (isProjectComplete) {
                        ...
                        if (calculatedStatuses['Team Assigned'] === 'done') {
                            if (lead.events && lead.events.length > 0) {
                                ...
                                if (hasCompleted) {
                                    ...
                                    calculatedDoneSteps.push('Outdoor Shoot Tracking', 'Shoot Completed');
                        ...
                        if (calculatedStatuses['Shoot Completed'] === 'done') {
                            ...
                            if (photoDelivery) {
                                ...
                                else if (['completed', 'approved', 'verified'].includes(status)) {
                                    calculatedStatuses['Photographer Upload'] = 'done';
                                    calculatedDoneSteps.push('Photographer Upload');
                                }
                            ...
                            if (videoDelivery) {
                                ...
                                if (status === 'rework') calculatedStatuses['Videographer Upload'] = 'reupload';
                                    ...
                                    calculatedStatuses['Videographer Upload'] = 'done';
                                    calculatedDoneSteps.push('Videographer Upload');
                                }
                        ...
                        const crmTasks = lead.leadEmployee?.filter((le: any) => {
                            ...
                            return tName.includes('retouch') || tName.includes('post') || tName.includes('video') || tName.includes('candid');
                        }) || [];

                        // Complete 'Work Status' if CRM tasks exist
                        const hasCrmTasks = crmTasks.length > 0;
                        ...
                        if (hasCrmTasks) {
                            ...
                            calculatedDoneSteps.push('Data Manager Verification', 'Assigned to CRM');
                    ...
                    if (lead.leadEmployee && lead.leadEmployee.length > 0) {
                        ...
                        if (crmTaskCount > 0) {
                            if (crmDoneCount === crmTaskCount) {
                                ...
                                calculatedDoneSteps.push('CRM Verified', 'Pre-production CRM Deliverables', 'Deliverables');
                            } else {
                    ...
                    if (calculatedStatuses['Pre-production CRM Deliverables'] === 'done' || calculatedDoneSteps.includes('Pre-production CRM Deliverables')) {
                        const assignmentCount = Object.keys(delivMap).length;
                        if (assignmentCount >= 4 && calculatedStatuses['Deliverables'] !== 'done') {
                            calculatedStatuses['Deliverables'] = 'done';
                            if (!calculatedDoneSteps.includes('Deliverables')) calculatedDoneSteps.push('Deliverables');
                        }
                    }
