...
const buildTracking = ({ hasAssignment, hasEvent, rawDeliveries, finalDeliveries, }) => {
...
const resolveWorkStatus = (leadStatus, rawDeliveries, finalDeliveries) => {
    const finalApproved = finalDeliveries.some((delivery) => normalizeStatus(delivery.status) === "client_approved");
    if (finalApproved)
        return "completed";
...
const buildProjectMeta = (lead, rawDeliveries, finalDeliveries) => {
    ...
    return {
        ...
        deliveryStatus: {
            ...
            incomingPending: rawDeliveries.filter((delivery) => ["pending", "query_raised"].includes(normalizeStatus(delivery.status))).length,
            ...
            finalPending: finalDeliveries.filter((delivery) => ["pending", "query_raised"].includes(normalizeStatus(delivery.status))).length,
...
const getClientWorks = async (req, res) => {
    try {
        ...
        const rawDeliveries = lead.clientDeliveries.filter((delivery) => ["RAW_DATA", "EVENT_RAW_DATA"].includes(delivery.deliveryType));
...
exports.getClientWorks = getClientWorks;
