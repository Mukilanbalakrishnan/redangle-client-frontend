...
exports.raiseClientQuery = exports.approveClientDelivery = exports.getClientDeliveries = void 0;
const client_1 = require("@prisma/client");
const axios_1 = __importDefault(require("axios"));
const auth_1 = require("../util/auth");
const prisma = new client_1.PrismaClient(); // Reload Prisma Client
const appendDeliveryNote = (currentNotes, label, message) => {
...
exports.getClientDeliveries = getClientDeliveries;
const approveClientDelivery = async (req, res) => {
    try {
        const token = req.headers.authorization?.split(" ")[1];
        if (!token)
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        ...
        try {
            ...
            await axios_1.default.patch(`${CRM_API_URL}/crm/client-deliveries/${serialNumber}/client-approve`, {
...
exports.approveClientDelivery = approveClientDelivery;
const raiseClientQuery = async (req, res) => {
    try {
        const token = req.headers.authorization?.split(" ")[1];
        if (!token)
            return res.status(401).json({ success: false, message: 'Unauthorized' });
        ...
        try {
            ...
            const serialNumber = lead?.leadSerialNumber || leadId;
            const CRM_API_URL = process.env.CRM_API_URL || 'http://localhost:5001/api';
            await axios_1.default.post(`${CRM_API_URL}/data-manager/${serialNumber}/client-query`, {
...
exports.raiseClientQuery = raiseClientQuery;
